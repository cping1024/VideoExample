...
NVENCSTATUS CNvHWEncoder::ProcessOutput(const EncodeBuffer *pEncodeBuffer, NV_ENC_LOCK_BITSTREAM* bitstream)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
    ...
    {
        if(bitstream) {
            bitstream->frameIdx = lockBitstreamData.frameIdx;
            bitstream->outputDuration = lockBitstreamData.outputDuration;
            bitstream->outputTimeStamp = lockBitstreamData.outputTimeStamp;
            bitstream->pictureType = lockBitstreamData.pictureType;
            bitstream->bitstreamSizeInBytes = lockBitstreamData.bitstreamSizeInBytes;
            memcpy(bitstream->bitstreamBufferPtr, lockBitstreamData.bitstreamBufferPtr, lockBitstreamData.bitstreamSizeInBytes);
        }
